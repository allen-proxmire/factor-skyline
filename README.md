...
The **Factor Skyline** reveals a simple but radical truth: **primes are not random**. They are the deterministic escape points of a geometric coverage process generated by the least prime factor, and every “mysterious” statistical feature of the integers — prime gaps, correlations, entropy, randomness tests, even GUE‑type behavior — emerges from the interaction of independent prime layers. This mathematical architecture turns multiplicative number theory into a visible, reproducible theory, where structure replaces mystery and every numerical claim can be regenerated from source.

The Factor Skyline is a geometric architecture for the integers in which each integer *n* is represented as a column of width lpf(*n*) (least prime factor) and height *n*/lpf(*n*), placed cumulatively along a horizontal axis. The resulting two-dimensional landscape — the skyline — is governed by five emergent concepts (width, height, activation, coverage, and escape), all deriving from the single primitive function lpf. This repository contains a four-part monograph developing the Factor Skyline as a unified architectural, statistical, dynamical, and meta-mathematical theory of multiplicative number theory, together with a deterministic reproducibility harness that regenerates every numerical result cited in the monograph. The goal is a fully reproducible mathematical platform: every theorem, every table, and every verification can be independently checked from source.

---

## Relationship to Architectural Distillation

The Factor Skyline is one instance of **[Architectural Distillation](https://github.com/allen-proxmire/architectural-distillation)** (AD), a general methodology for extracting structure from systems composed of interacting mechanisms. AD provides the concepts (channels, envelopes, constraint surfaces, poles) and the step-by-step process; FS is a specific mathematical theory produced by applying that process to the multiplicative structure of the integers.

- **Upstream methodology:** FS was originally developed alongside AD, which now lives in its own [dedicated repository](https://github.com/allen-proxmire/architectural-distillation). AD defines the general distillation process; FS inherits its architectural vocabulary from AD.
- **Downstream output:** FS represents one fully developed mathematical architecture produced using AD. The definitions, propositions, proofs, invariants, and reproducibility harness in this repository are the domain-specific mathematical content — the product of the distillation, not the methodology itself.

For the general methodology — applicable to any system with interacting mechanisms — see the [AD repository](https://github.com/allen-proxmire/architectural-distillation).
...
This architecture unifies the four papers: Part I builds the geometric foundations, Part II derives correlation and randomness structure from CRT independence of width layers, Part III establishes entropy, ergodic, and universality theories with D(*p*) as a renormalization coupling constant, and Part IV examines the theory's own meta-structure, identifying the parity barrier as the precise limit of the architecture's deductive reach.
